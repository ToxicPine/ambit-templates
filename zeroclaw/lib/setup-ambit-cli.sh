...
# Idempotent setup of ambit-cli and system context for zeroclaw.
# Runs as a regular user with HOME, git, and nix on PATH.

ZEROCLAW_DIR="$HOME/.config/zeroclaw"
MARKER="$ZEROCLAW_DIR/.ambit-cli-installed"
...
SKILLS_DIR="$ZEROCLAW_DIR/skills"
mkdir -p "$SKILLS_DIR"
...
# --- System details document ---
mkdir -p "$ZEROCLAW_DIR/context"
cat > "$ZEROCLAW_DIR/context/system-details.md" << 'CTXEOF'
# System Details
